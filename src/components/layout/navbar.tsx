...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 glass-morphism">
          ...
          {mounted && (
            <div className="md:hidden">
              <Sheet>
                ...
                <SheetContent side="right" className="bg-background border-l border-border/50 w-[300px] p-0">
                    ...
                    <nav className="flex flex-col space-y-6 text-center">
                      {navLinks.map((link) => (
                        <Link 
                          key={link.label} 
                          href={link.href} 
                          className="text-sm font-medium uppercase tracking-[0.3em] hover:text-accent transition-colors py-2 border-b border-border/50"
                        >
                          {link.label}
                        </Link>
                      ))}
